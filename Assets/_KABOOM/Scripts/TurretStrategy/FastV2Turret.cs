...
/// </summary>
public class FastV2Turret : MonoBehaviour
{
    public IShoot CurrentShotType; // The specific type of shooting to be used

    public static FastV2Turret SharedInstance; // For the bullet pool
    ...
    protected float _shotSpeed = 3f; // How fast the bullet comes out

    protected float _shotDelay = 3f; // Time between shots

    protected int _amountOfProjectiles = 5; // How many bullets per shot

    protected bool _isHoming = false; // Whether the bullets track the player or not

    protected int _bulletPoolAmount = 15; // How many bullets pooled. KEEP HIGH ENOUGH for the speed and amount of projectiles to avoid out of range exceptions

    protected float _bulletLifetime = 5f; // How long until bullets are set inactive

    protected float _shotStagger = 0.4f; // Stagger shots in case of shooting multiple bullets at a time
